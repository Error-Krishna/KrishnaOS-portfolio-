...
const WELCOME_LINES: TerminalLine[] = [
  { type: 'system', text: 'krishnaos-terminal v1.0 — type "help" to see available commands' },
];
...
export function AboutTerminal() {
  ...
  const runCommand = (raw: string) => {
    ...
    setHistory((prev) => [
      ...
      ...(command
        ? command.run().map((line): TerminalLine => ({ type: 'output', text: line }))
        : [{ type: 'output' as const, text: `command not found: ${name} — type "help"` }]),
    ]);
  };
  ...
  return (
    <div
      className="flex h-full min-h-[260px] flex-col overflow-hidden rounded-os-lg border border-[color:var(--color-os-glass-border)] bg-[#0a0a0c] font-mono text-os-caption"
    ...
    >
      <div className="flex items-center gap-os-2 border-b border-[color:var(--color-os-glass-border)] bg-[color:var(--color-os-glass)] px-os-3 py-os-2">
        ...
        <span className="ml-os-2 truncate text-[color:var(--color-os-text-tertiary)]">
          building_better_v1.0
        </span>
      </div>

      <div ref={scrollRef} className="flex-1 space-y-1 overflow-y-auto px-os-3 py-os-3" aria-live="polite">
        {history.map((line, i) => (
          <p
            key={i}
            className={
      ...
      >
        ...
        <input
          ...
          className="flex-1 bg-transparent text-[color:var(--color-os-text-primary)] outline-none placeholder:text-[color:var(--color-os-text-tertiary)]"
        />
      </form>
    </div>
  );
